        ...
        <SafeAreaView style={styles.container}>
            <View style={styles.maincontainer}>
                <View style={{width: width}}>
                    <Animated.FlatList renderItem={renderSongs} data={songspop} keyExtractor={(item) => item.id}
                        horizontal pagingEnabled showsHorizontalScrollIndicator={false} 
                        scrollEventThrottle={16} onScroll={Animated.event([{ nativeEvent: { contentOffset: {
                            x: scrollX
                        }}}], {useNativeDriver: true })} ref={songSlider}/>
                    <Text style={styles.title}>{songspop[songIndex].title}</Text>
                    <Text style={styles.artist}>{songspop[songIndex].artist}</Text>
                </View>
                <View>
